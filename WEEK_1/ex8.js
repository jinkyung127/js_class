...
function order(item, quantity, price) {
    ...
    if (item.length<=0 || quantity.length<=0 || price<=0) {
        return console.log('주문 정보를 모두 입력해주세요');
    } else if (quantity <= 0) {
        return console.log('수량은 1개 이상 입력해주세요');
    } else {
        return console.log(`주문이 완료되었습니다. ${item} ${quantity}개를 ${price}원에 구매하셨습니다.`);
    }
	// quantity 수량이 0이하면
